...
gsutil -m cp  gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-scores/**/finngen*log $OUT_PATH/release/documentation/

#echo "SCORES"
wc -l <  <( gsutil ls gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-scores/**/finngen*sscore)
gsutil -m cp         gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-scores/**/finngen*sscore $OUT_PATH/release/data/

# WEIGHTS
echo "WEIGHTS"
wc -l < <(gsutil ls  gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-weights/**/finngen*weights.txt)
gsutil -m cp gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-weights/**/finngen*weights.txt $OUT_PATH/release/data/

# MUNGED
echo "MUNGED SUMSTATS"
wc -l < <(gsutil ls gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-weights/**/munge/finngen*rsid)
gsutil -m cp gs://$CROMWELL_NAME/prs_cs/$CROMWELL_ID/call-weights/**/munge/finngen*rsid $OUT_PATH/munged_rsid/
