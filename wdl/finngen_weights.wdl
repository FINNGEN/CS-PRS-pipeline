...
task munge_sumstats {
  ...
  command <<<
    zcat ~{pheno_file} | awk -F'\t' -v OFS="\t" '{ print "chr"$1"_"$2"_"$3"_"$4, $4, $3, $8, $7}' |\
    { head -n1| awk -v OFS="\t" '{ print "SNP",$2,$3,$4,$5 }' ; \
    grep -wf <( cut -f2 ~{bim_file})  ~{true=" | shuf | head -n 10000 " false="" test}; } \
    > ~{prefix}_~{pheno}.munged
  >>>
